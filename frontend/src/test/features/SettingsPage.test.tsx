vi.mock('../../features/data/SourcePolicyEditor', () => ({ SourcePolicyEditor: () => null }))
vi.mock('../../features/settings/IncomeExpectationsEditor', () => ({ IncomeExpectationsEditor: () => null }))
...
describe('SettingsPage', () => {
  ...
  it('backend가 반환하는 분석 설정 contract 섹션을 모두 렌더링한다', () => {
    renderPage()
    for (const name of ['이상 지출', '재량 지출 속도', '구매 검토 후보', '반복 결제 후보 검토', '자산·부채 분석', '일괄 작업 확인']) {
      expect(section(name)).toBeInTheDocument()
    }
    expect(within(section('이상 지출')).queryByText('기본 100,000 · 저장 미설정 · 적용 100,000')).not.toBeInTheDocument()
    ...
    expect(within(section('일괄 작업 확인')).getByRole('button', { name: '설명: 추가 확인 없는 최대 행 수' })).toBeInTheDocument()
    expect(within(section('일괄 작업 확인')).queryByRole('checkbox')).not.toBeInTheDocument()
    expect(within(section('일괄 작업 확인')).queryByRole('spinbutton')).not.toBeInTheDocument()
  })
  ...
  it('T013 남은 분석 설정 섹션마다 편집 컨트롤을 제공한다', () => {
    ...
    expect(getByLabelPrefix('신규 가맹점 조회 기간 (개월)')).toHaveValue(6)
    expect(getByLabelPrefix('가맹점 급증 기준 (%)')).toHaveValue(200)
    expect(getByLabelPrefix('재량 지출 급증 기준 (%)')).toHaveValue(150)
    expect(getByLabelPrefix('리뷰 재검토 간격 (일)')).toHaveValue(14)
    ...
    expect(getByLabelPrefix('비상금 포함 유동성 등급')).toHaveValue('immediate')
    expect(getByLabelPrefix('단기 현금화 자산 보조 표시')).toBeChecked()
    expect(getByLabelPrefix('월상환 추정 조회 기간 (개월)')).toHaveValue(6)
  ...
  it('토글과 리스트 분석 설정에도 기본/저장/적용 helper 값을 표시한다', () => {
    ...
    openHelp('설명: 비상금 포함 유동성 등급')
    expect(screen.getByText('비상금 계산에 포함할 자산 유동성 등급 목록입니다.')).toBeInTheDocument()
    expect(screen.getByText('기본 즉시 현금화 · 저장 미설정 · 적용 즉시 현금화')).toBeInTheDocument()

    openHelp('설명: 단기 현금화 자산 보조 표시')
  ...
  it('T013 분석 설정 저장 payload가 모든 편집 섹션과 퍼센트 변환을 포함한다', async () => {
    ...
    changeField('신규 가맹점 조회 기간 (개월)', '8')
    changeField('가맹점 급증 기준 (%)', '250')
    changeField('재량 지출 급증 기준 (%)', '175')
    changeField('리뷰 재검토 간격 (일)', '30')
    changeField('후보 위험 임계값', 'high')
    fireEvent.click(within(section('구매 검토 후보')).getByLabelText('신규 가맹점'))
    fireEvent.change(within(section('구매 검토 후보')).getByLabelText((content) => content.startsWith('제외 카테고리')), { target: { value: '여행, 쇼핑' } })
    fireEvent.change(within(section('구매 검토 후보')).getByLabelText((content) => content.startsWith('제외 가맹점')), { target: { value: '면세점, 백화점' } })
    changeField('재량 기준 기간 (개월)', '4')
    changeField('재량 속도 경고 (%)', '130')
    changeField('재량 속도 높음 (%)', '180')
    changeField('분류 커버리지 최소 (%)', '80')
    changeField('이상값 처리 방식', 'none')
    changeField('비교 기준 방식', 'closed_month')
    fireEvent.change(within(section('재량 지출 속도')).getByLabelText((content) => content.startsWith('제외 카테고리')), { target: { value: '식비, 여가' } })
    fireEvent.change(within(section('재량 지출 속도')).getByLabelText((content) => content.startsWith('제외 가맹점')), { target: { value: '쿠팡' } })
    ...
    changeField('비상금 포함 유동성 등급', 'immediate, near_liquid')
    fireEvent.click(getByLabelPrefix('단기 현금화 자산 보조 표시'))
    changeField('월상환 추정 조회 기간 (개월)', '12')
  ...
  it('빈 필수 숫자 분석 설정은 검증 오류를 보이고 PATCH하지 않는다', async () => {
    renderPage()
    fireEvent.change(getByLabelPrefix('리뷰 재검토 간격 (일)'), { target: { value: '' } })
    fireEvent.click(screen.getByRole('button', { name: '분석 설정 저장' }))

    await waitFor(() => {
      expect(screen.getByText('리뷰 재검토 간격은 숫자여야 합니다')).toBeInTheDocument()
    })
    expect(patchMutate).not.toHaveBeenCalled()
  })
