...
describe('TransactionsTable', () => {
  ...
  it('keeps the mobile merchant block shrinkable so the amount stays inside the card', () => {
    render(
      <TransactionsTable
        rows={[
          {
            ...
            category_minor_user: null,
            description: '생활비 자동이체 테스트',
            merchant: '아주 긴 거래처 이름 아주 긴 거래처 이름 아주 긴 거래처 이름',
            effective_category_major: '생활',
            effective_category_minor: '고정비',
            payment_method: '우리카드',
            amount: -25000,
    ...
    const merchantHeading = screen
      .getAllByText('아주 긴 거래처 이름 아주 긴 거래처 이름 아주 긴 거래처 이름')
      .find((element) => element.tagName === 'P');
    expect(merchantHeading).toBeDefined();
    if (!merchantHeading) {
      throw new Error('Expected mobile merchant heading');
    }
    const mobileRow = merchantHeading.closest('.flex');

    expect(merchantHeading.parentElement?.className).toContain('min-w-0');
    expect(merchantHeading.className).toContain('truncate');
    expect(mobileRow?.lastElementChild?.className).toContain('shrink-0');
    expect(mobileRow?.lastElementChild?.className).toContain('text-right');
  });
});
