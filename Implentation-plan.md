...
7. 2026-09-24 Live 점검 후 `T042` 대출 연결 거래 월상환 추정 복구와 `T043` 진행월 예상 수입·월말 순현금흐름을 추가했다. 후속 사용자 승인으로 `.omo/plans/live-audit-remediation.md`에서 구현·검증·배포를 진행 중이다. 구현 중 상태는 Live 복구 완료를 의미하지 않는다. `T043`은 `T037` 전체 90일 calendar보다 좁은 월간 선행 작업이다.
...
| Live 추정값 정합성과 진행월 예측 | `T042`-`T043` | 진행 중; Live 점검 수정 실행계획에서 구현·검증 |
...
- 참고:
  ...
  - 진행월 급여 미입금과 예상 월수입·월말 순현금흐름은 `T043`에서 먼저 다룬다. 이 task는 그 수입 예상/실제 입금 대조를 재사용하며, 기존 90일 일별 잔액·예정 지출 요구사항을 유지한다.
...
## Live 점검 후 추가된 수정·보완 항목

#### 작업 T042. 대출 연결 거래 월상환 추정 저장·재계산·화면 갱신 복구
- 우선순위: P0
- 상태: 진행 중
- 선행 조건: T005, T030, T031
- 목표: 거래 연결이 있어도 추정액이 사라지거나 과거 값으로 남는 문제를 고치고, 실제 수동 설정은 보존한다.
- 확인된 문제:
  - 계좌명·대출 성격 저장 시 편집하지 않은 빈 수동 월상환액이 `monthly_payment=null`로 함께 전송될 수 있다. 서버는 이를 `monthly_payment_source='manual'`로 저장하고 이후 자동 추정을 차단한다.
  - 자동 대출 규칙 적용은 거래 연결을 생성·변경하지만 관련 계좌의 월상환 추정치를 재계산하지 않는다.
  - 일괄 연결과 자동 규칙 적용 후 거래 query만 갱신하여 대출 요약 카드가 오래된 값을 유지할 수 있다.
- 완료 기준:
  - [ ] 계좌명·종류만 저장하면 편집하지 않은 월상환액·상환 방식과 각 source가 바뀌지 않는다. 비동기 응답 순서가 달라도 기존 상환 필드를 빈값으로 덮어쓰지 않는다.
  - [ ] 실제 수동 월상환액은 자동 계산보다 우선하고, 수동 설정 해제 및 자동 추정 복귀는 명시적인 API/UI 동작으로 구분한다. 기존 `null` 계약을 조용히 재해석하지 않는다.
  - [ ] 자동 규칙으로 링크가 생성·변경되면 변경 전후 계좌의 최신 스냅샷 추정액을 재계산한다. 수동 연결·연결 해제·계좌 이동과 같은 결과를 보장한다.
  - [ ] 수동 일괄 연결과 자동 규칙 적용 성공 후 대출 계좌·요약·관련 분석 query가 필요한 범위에서 갱신된다.
  - [ ] 추정값 없음의 이유를 수동 설정, 관측 월 부족, 진행월 제외, 연결 없음 등으로 구분해 설명하고 관측 구간·계산 근거를 제공한다.
  - [ ] 회귀 검증에 계좌명만 저장, 상환 방식만 편집, 실제 수동 금액 보존, 수동 설정 해제, 자동 연결 추가·계좌 이동, 최신 요약 표시를 포함한다.
  - [ ] 기존 잘못된 `null/manual`과 오래된 추정값의 복구 대상을 읽기 전용으로 선별한다. 배포 확인 후 승인된 API 경로로 복구하고, 실제 수동값과 거래 연결을 보존한 채 전후 GET 결과를 대조한다. 직접 DB 수정은 사용하지 않는다.
- 검증 및 참고:
  - 2026-09-24 Live GET·readonly DB·배포된 순수 계산 함수 대조로, 충분한 연결 거래가 있는 계좌의 `null/manual` 상태와 자동 연결 추가 후 저장 추정액 불일치를 확인했다. 개인별 금액·거래 원문은 버전 관리되는 계획 문서에 넣지 않는다.
  - 코드 경로: `frontend/src/features/data/LoansPage.tsx`, `frontend/src/hooks/useTransactions.ts`, `backend/app/services/assets_service.py`, `backend/app/services/loan_mapping_service.py`, `backend/app/services/auto_classification_service.py`.
  - 실행 전 별도의 좁은 OMO 계획과 API 의미·기존 데이터 복구 범위를 확정한다. 코드 구현, 테스트 통과, 배포, 기존 데이터 복구는 각각 별도 완료 근거로 기록한다.

#### 작업 T043. 진행월 예상 수입과 월말 순현금흐름 분리 표시
- 우선순위: P1
- 상태: 진행 중
- 선행 조건: T007, T023, T024, T032
- 목표: 급여 입금 전에도 과거 정기 수입을 근거로 월 전체 수입을 예상하고, 실제 입금·아직 들어오지 않은 예상 수입·월말 전망을 구분해 보여준다.
- 확인된 현재 동작:
  - canonical dashboard는 진행월 관측 수입이 과거 수입 baseline의 50%보다 작으면 추정 수입과 추정 가용액을 제공한다. 홈의 수입 카드와 월간 현금흐름 차트는 관측값만 소비한다.
  - 현재 보정은 과거 월 총수입 기반이며 급여 입금 여부·급여일을 직접 판단하지 않는다. 보험금·중고판매 등 비정기 수입이 baseline 또는 임계값에 영향을 줄 수 있다.
  - 현재 추정 가용액은 추정 수입에서 지금까지 기록된 지출을 차감한 보조값이다. 남은 예정 지출을 반영한 월말 순현금흐름이나 은행 잔액 예측으로 해석하지 않는다.
- 완료 기준:
  - [ ] `observed_income`, `expected_remaining_income`, `projected_month_income` 또는 동등한 필드를 분리하고, 월 예상 수입은 실제 수입 + 아직 관측되지 않은 예상 수입으로 구성한다. 관측 거래·canonical 실적 금액은 수정하지 않는다.
  - [ ] 급여 등 정기 수입을 입금처·사용자 분류·금액 안정성·주기·입금일 범위로 판별하고, 사용자가 금액·예정일·중단 여부를 확인하거나 보정할 수 있다. 소액 급여 정산·보너스·보험금·중고판매·환급을 매월 반복 수입으로 단순 전개하지 않는다.
  - [ ] 최근 완료월의 범위, 실제 데이터 coverage, 누락 월, 이상치 제외, 기준일을 명시한다. 단순히 현재 월보다 과거라는 이유로 부분 수집 월을 완료월로 취급하지 않는다.
  - [ ] 실제 급여가 들어오면 해당 예상 입금과 대조해 중복 합산하지 않는다. 분할 입금·복수 입금처·금액 변경·휴일 전후 입금·입금 지연·퇴사 또는 중단·이력 부족을 검증하고, 불확실한 일치는 사유와 검토 상태를 남긴다.
  - [ ] 수입 카드·현금흐름 차트·순현금흐름 표시에서 실적과 전망을 함께 식별할 수 있다. 수입에만 예상값을 넣고 순현금흐름은 실적값인 상태를 동일 기준처럼 표시하지 않는다.
  - [ ] 월말 순현금흐름은 월 예상 수입 - (관측 순지출 + 잔여 예상 순지출)로 계산하며, 실제 거래와 예정 대출·할부·반복결제·변동 지출의 중복 차감을 방지한다. 남은 지출 입력이 부족하면 그 한계를 표시하고 단순히 현재까지 지출을 뺀 값을 월말 전망이라고 부르지 않는다.
  - [ ] 대출 월상환 추정치를 잔여 지출 입력으로 사용할 때는 `T042`의 정합성 복구가 선행되어야 한다. `T036`의 반복계약은 완성 후 연결할 수 있도록 경계를 유지하며, 미제공 입력을 확인된 예정 금액처럼 취급하지 않는다.
  - [ ] 응답과 화면에 추정 근거, 포함/제외 기간, 예정일 또는 범위, 기준일, 최신 업로드 coverage, confidence, 누락 사유를 표시한다. 부분월 수입 임계값만으로 정기 수입 예측을 켜거나 끄지 않는다.
  - [ ] 회귀 검증에 급여 전 소액 수입, 급여 실제 입금 후 예상 제거, 급여 전 큰 일시수입, 분할·변동 급여, 지연·중단, 부분월 데이터, 잔여 지출·중복 제거를 포함한다. 월말 전망과 현재 현금 잔액은 별개임을 API·화면에서 확인한다.
- 검증 및 참고:
  ...
  - 실행 전 월간 범위의 별도 OMO 계획을 확정한다. `T033`-`T036` 전체 자동화 묶음 완료를 이 작업의 일괄 선행 조건으로 추가하지 않는다.
