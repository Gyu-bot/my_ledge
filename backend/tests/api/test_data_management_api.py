...
async def test_reset_data_clears_transactions_only_and_retains_snapshots_and_logs(
    ...
    before_asset_snapshots = await db_session.scalar(
        select(func.count()).select_from(AssetSnapshot)
    )
    before_insurance_contracts = await db_session.scalar(
        select(func.count()).select_from(InsuranceContract)
    )
    before_investments = await db_session.scalar(
        select(func.count()).select_from(Investment)
    )
    before_loans = await db_session.scalar(select(func.count()).select_from(Loan))
    before_upload_logs = await db_session.scalar(
    ...
    remaining_asset_snapshots = await db_session.scalar(
        select(func.count()).select_from(AssetSnapshot)
    )
    remaining_insurance_contracts = await db_session.scalar(
        select(func.count()).select_from(InsuranceContract)
    )
    remaining_investments = await db_session.scalar(
        select(func.count()).select_from(Investment)
    )
    remaining_loans = await db_session.scalar(select(func.count()).select_from(Loan))
    remaining_upload_logs = await db_session.scalar(
        select(func.count()).select_from(UploadLog)
    )

    assert remaining_transactions == 0
    assert remaining_asset_snapshots == before_asset_snapshots
    assert remaining_insurance_contracts == before_insurance_contracts
...
async def test_reset_data_clears_transactions_and_snapshots(
    ...
    assert remaining_insurance_contracts == 0
