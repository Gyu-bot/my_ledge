...
async def test_purchase_gate_candidates_include_spike_types_and_saved_review_status(
    ...
    assert (
        reviewed_payload["items"][0]["candidate_key"] == merchant_spike["candidate_key"]
    )
...
async def test_purchase_gate_candidates_net_full_refunds_out_of_review_queue(
    ...
    db_session.add(
        SettlementMatch(
            ...
            matched_amount=150_000,
            matched_at=datetime(2026, 4, 12, 10, 1, 0),
        )
    )
    await db_session.commit()
...
async def test_monthly_cashflow_endpoint_uses_confirmed_settlement_net_amount(
    async_client: AsyncClient,
    db_session: AsyncSession,
) -> None:
    purchase = _transaction(
...
async def test_monthly_cashflow_endpoint_keeps_confirmed_settlement_read_only(
    async_client: AsyncClient,
    db_session: AsyncSession,
) -> None:
    purchase = _transaction(
    ...
    db_session.add(
        SettlementMatch(
            ...
            status=SettlementMatchStatus.AUTO_CONFIRMED.value,
            matched_amount=80_000,
            matched_at=datetime(2026, 2, 1, 12, 31, 0),
        )
    )
    await db_session.commit()
...
async def test_category_mom_endpoint_keeps_rejected_settlement_on_raw_basis(
    async_client: AsyncClient,
    db_session: AsyncSession,
) -> None:
...
async def test_fixed_cost_trend_endpoint_returns_monthly_totals(
    ...
    assert response.json()["items"] == [
        ...
        },
        {
            ...
            "essential_variable_total": 0,
            "discretionary_variable_total": 0,
            "required_spend_total": 0,
            "discretionary_spend_total": 50,
            "unclassified_total": 0,
            "unclassified_count": 0,
            "fixed_ratio": 1.0,
        },
...
async def test_merchant_spend_endpoint_uses_confirmed_settlement_net_amount(
    async_client: AsyncClient,
    db_session: AsyncSession,
) -> None:
    purchase = _transaction(
        ...
        "items": [
            {
                ...
                "count": 1,
                "avg_amount": 80.0,
                "last_seen_at": "2026-01-01T10:00:00",
            },
            {
                "merchant": "넷플릭스",
                "amount": 50,
...
async def test_income_stability_endpoint_defaults_to_last_closed_month(
    ...
    assert response.status_code == 200
    data = response.json()
    assert [item["period"] for item in data["items"]] == [
        "2026-01",
        "2026-02",
        "2026-03",
    ]
