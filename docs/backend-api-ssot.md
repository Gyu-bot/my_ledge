...
- Components retain `known_expected_remaining`; the total is `known_expected_remaining_expense`. `net_after_known_remaining_expense` subtracts only those known amounts and omits unknown expense, so it is a partial calculation rather than the full month-end forecast or a cash balance.
