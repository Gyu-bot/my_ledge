...
- [ ] `분석 > 지출` 의 시계열 날짜 필터는 slider 조작 즉시 반영하지 않고 `적용` 버튼 클릭 시에만 실제 필터링을 실행한다
- [ ] `분석 > 지출` 의 월 필터는 `시작 월`, `종료 월`만 유지하고 `카테고리`, `결제수단`, `설명 검색` 필터는 제거한다
- [ ] `분석 > 지출` 의 `일별 지출액` 카드는 페이지 공용 상세 필터와 분리하고 카드 내부 dropdown filter로 독립 제어한다
- [ ] `분석 > 지출` 의 `결제수단별 지출` 카드는 제거하고 `거래처별 TreeMap` 카드는 전체 폭으로 확장한다
- [ ] `분석 > 자산` 의 `투자 요약` 카드는 상단 `총 투자원금`, `총 평가액` 요약을 유지하고 하단은 항목별 비중 pie chart로 재구성한다
- [ ] 프로젝트 전역에서 `기준 월`, `기준 일자` 등 기준값 badge/pill 배경색 체계를 하나로 통일한다
- [ ] `거래 편집 작업대` 성능 최적화 배치를 별도 후속 작업으로 잡고, 서버 필터링/페이지네이션 중심으로 재구성한다

**Notes**
- 위 항목은 다음 구현 배치용 backlog이며 이번 턴에는 문서에만 추가한다
- 지출 페이지의 시계열 slider는 draft/apply 2단계 상태로 바꾸고, 버튼 클릭 전에는 실제 query key를 바꾸지 않는다
- 지출 상세 필터는 월 범위만 남기고 나머지 보조 필터는 제거해 정보 구조를 단순화한다
- 작업대 성능 최적화는 UI 미세 조정보다 우선순위를 높게 두고, `전체 페이지 수집 -> 클라이언트 필터링` 구조 제거를 목표로 둔다
