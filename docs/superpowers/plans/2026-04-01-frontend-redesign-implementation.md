...
- [x] `분석 > 지출` 의 시계열 날짜 필터는 slider 조작 즉시 반영하지 않고 `적용` 버튼 클릭 시에만 실제 필터링을 실행한다
- [x] `분석 > 지출` 의 월 필터는 `시작 월`, `종료 월`만 유지하고 `카테고리`, `결제수단`, `설명 검색` 필터는 제거한다
- [ ] `분석 > 지출` 의 `일별 지출액` 카드는 페이지 공용 상세 필터와 분리하고 카드 내부 dropdown filter로 독립 제어한다
- [x] `분석 > 지출` 의 `결제수단별 지출` 카드는 제거하고 `거래처별 TreeMap` 카드는 전체 폭으로 확장한다
- [x] `분석 > 자산` 의 `투자 요약` 카드는 상단 `총 투자원금`, `총 평가액` 요약을 유지하고 하단은 항목별 비중 pie chart로 재구성한다
- [x] 프로젝트 전역에서 `기준 월`, `기준 일자` 등 기준값 badge/pill 배경색 체계를 하나로 통일한다
- [x] `거래 편집 작업대` 성능 최적화 배치를 별도 후속 작업으로 잡고, 서버 필터링/페이지네이션 중심으로 재구성한다

**Notes**
- 완료된 항목은 2026-04-02 배치에서 실제 구현과 검증까지 끝냈다
- 남은 우선 backlog는 `일별 지출액` 카드의 독립 dropdown filter와 `cost_kind` / `fixed_cost_necessity` rule-based 자동 분류 설계다
