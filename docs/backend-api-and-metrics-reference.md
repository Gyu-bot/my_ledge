...
| `confidence`, `missing_reasons`, `limitations` | Overall model/evidence limits; null totals must not become zero. |
...
| `expense_components[]` | `kind`, nullable `expected_remaining`, `known_expected_remaining`, `basis`, `missing_reasons`. |
...
3. Stable recurring payer net expenses use at least three covered months and the latest covered month's evidence, subtract current observed net expense and floor at zero. Explicit monthly-recurring activity without adequate history remains missing. Non-recurring fixed classifications without a stable pattern move to the residual model.
