...
개발/검증용 샘플:
- 현재 로컬 샘플 파일은 `/tmp/finance_sample.xlsx` 이며 비암호화 상태다.
...
├─────────────────────────────────────────┤
│ id              SERIAL PK               │
│ snapshot_date   DATE NOT NULL            │  -- API 입력값 우선, 없으면 서버 업로드 날짜
...
│ snapshot_date   DATE                     │  -- API 입력값 우선, 없으면 서버 업로드 날짜
...
- `snapshot_date` 결정 규칙:
  1. API에서 `snapshot_date`를 전달하면 그 값을 사용
  2. 미전달 시 서버 업로드 날짜를 사용
  3. 이 값은 `asset_snapshots`, `investments`, `loans`, `upload_logs.snapshot_date`에 동일하게 기록
...
Parameters:
  ...
  snapshot_date: string (optional, YYYY-MM-DD, 미제공 시 서버 업로드 날짜)
