...
my_ledge v2는 기존 my_ledge 프로젝트를 리셋·확장하는 **개인 재무 대시보드**다.
BankSalad에서 내보낸 엑셀 파일을 데이터 소스로 사용하여 지출 분석과 자산 변동 tracking을 수행하고, OpenClaw 멀티에이전트 시스템과 연동하여 LLM 기반 심층 분석을 지원한다.
...
| 결제수단 | `string` | 카드/계좌 이름 (∼20종) | 현대카드Z family |
...
├─────────────────────────────────────────┤
│ id              SERIAL PK               │
│ snapshot_date   DATE NOT NULL            │  -- 업로드 시점 기록
...
| 레이어 | 기술 | 비고 |
|---|---|---|
| Frontend | React + Vite + Recharts + Tailwind CSS | my_ledge v1 스택 유지 |
...
Parameters:
  ...
  snapshot_date: string (optional, YYYY-MM-DD, 미제공 시 업로드 시점)
...
- **결제수단별 지출 분포** (바 차트)
- **일별 지출 캘린더 히트맵**
- **날짜 범위 필터 + 카테고리/결제수단 필터**
- **거래 내역 테이블** (정렬, 검색, 페이지네이션)
...
| 분석 | 설명 | 필요 데이터 |
|---|---|---|
| **고정비 vs 변동비 분류** | 구독, 보험, 주거/통신 = 고정비; 나머지 = 변동비 | 대분류 기반 자동 분류 |
...
- 업로드된 원본 엑셀 파일: `/data/uploads/` 디렉토리에 보관
...
| 결제수단 | 건수 | 비중 |
|---|---|---|
| 현대카드Z family | 575 | 25.9% |
| American Express Gold Card Edition2 | 292 | 13.2% |
| KB국민ONE통장-저축예금 | 258 | 11.6% |
| (구)KB국민 My WE:SH 카드 | 244 | 11.0% |
| 삼성스토어 BENEFIT 삼성카드 | 236 | 10.6% |

## 부록 B: 뱅샐현황 재무현황 구조 (현재 스냅샷 기준)

### 자산 측

| 카테고리 | 항목 수 | 합계(원) |
|---|---|---|
| 자유입출금 자산 | 10 | ∼1,115,083 |
| 저축성 자산 | 1 | 6,180,000 |
| 전자금융 자산 | 4 | 0 |
| 투자성 자산 | 16 | ∼16,254,104 |
| 부동산 | 1 | 270,000,000 |
| 보험 자산 | 2 | ∼1,480 |
| 연금 자산 | 1 | 47,900,499 |
| **총자산** | | **∼341,467,220** |
