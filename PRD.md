...
│ merchant        VARCHAR(500) NOT NULL    │  -- 거래처 표시명 (기본값=description)
...
**뱅샐현황 (스냅샷):** `snapshot_date` 단위 replace
- 현재 구현은 같은 `snapshot_date` 의 기존 snapshot row를 먼저 삭제하고, 새 파싱 결과 전체를 다시 INSERT 한다.
- 즉, 문서상 UPSERT 의도와 달리 live behavior는 date-scoped replace다.
- `snapshot_date` 결정 규칙:
...
|---|---|---|---|
| `GET /api/v1/analytics/recurring-payments` | `merchant`, `category`, `avg_amount`, `interval_type`, `avg_interval_days`, `occurrences`, `confidence`, `last_date` | description/merchant 그룹 + 날짜 간격 휴리스틱 | live |
| `GET /api/v1/analytics/spending-anomalies` | `period`, `category`, `amount`, `baseline_avg`, `delta_pct`, `anomaly_score`, `reason` | 최근 N개월 baseline + category rule | live |
| `GET /api/v1/analytics/payment-method-patterns` | `payment_method`, `total_amount`, `transaction_count`, `avg_amount`, `pct_of_total` | `payment_method` + effective category 집계 | live |
| `GET /api/v1/analytics/income-stability` | 월 수입 시계열 + `avg`, `stdev`, `coefficient_of_variation`, `assumptions` | 월별 수입 집계 후 통계 계산 | live |
...
#### 6.1.2 지출 분석 (`/analysis/spending`, legacy `/spending` redirect)
...
#### 6.1.3 수입 분석 (`/income`, 현재 overview redirect)

- **월별 수입 추이** (급여 vs 기타 수입)
- **수입원별 분포**

#### 6.1.4 자산이동 (`/transfers`, 현재 overview redirect)

- **이체 타입별 월간 추이** (카드대금, 저축, 투자 등)
- **이체 내역 테이블**

#### 6.1.5 자산 현황 (`/analysis/assets`, legacy `/assets` redirect)
...
#### 6.1.6 거래 작업대 (`/operations/workbench`, legacy `/data` redirect)
